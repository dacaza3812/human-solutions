-- Drop existing referrals table if it exists to start fresh
...
CREATE OR REPLACE FUNCTION handle_referrals_updated_at()
RETURNS TRIGGER AS $$
BEGIN
  NEW.updated_at = NOW();
  RETURN NEW;
...
CREATE TRIGGER handle_referrals_updated_at
  ...
  FOR EACH ROW EXECUTE FUNCTION handle_referrals_updated_at();
...
-- This will only work if there are existing profiles
DO $$
DECLARE
...
BEGIN
  ...
  IF profile_count > 0 THEN
    -- Insert sample referral data only if profiles exist
    INSERT INTO public.referrals (referrer_id, referred_id, referral_code, status, commission_earned, commission_paid)
    SELECT 
    ...
    WHERE p1.id != p2.id 
      AND p1.referral_code IS NOT NULL
      AND NOT EXISTS (
    ...
    RAISE NOTICE 'Sample referral data inserted successfully';
  ELSE
    RAISE NOTICE 'No profiles found, skipping sample data insertion';
  END IF;
END $$;
